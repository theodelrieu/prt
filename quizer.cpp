...
#include "range.hpp"
inline auto constexpr correctAnswerText =
    "<font color=\"green\"><b>Correct!</b></font>";
inline auto constexpr wrongAnswerText =
...
Quizer::Quizer(RangeDisplayer const* displayer, QObject* parent)
        ...
        std::remove_if(_quizHands.begin(), _quizHands.end(),
                       [&](auto const& hand) {
                         ...
                         auto it = std::find_if(
                             ...
                             [&](auto const& sub) {
                               // TODO use == when weight is moved out of
                               // RangeInfo
                               return sub.name() == excluded.name() &&
                                      sub.color() == excluded.color();
                             });
                         if (it != subranges.end()) return it->weight() == 100;
                         return false;
                       }),
        _quizHands.end());
  }
...
auto Quizer::nextQuizParams() -> std::tuple<QuizType, HandInfo const*, int> {
  ...
  return {static_cast<QuizType>(1 + quizTypeDistrib(_rng)),
          std::addressof(hand), gridIdx};
}
...
void Quizer::nextInSubrangeQuiz(HandInfo const* hand, int handIndex) {
  ...
  auto const hasSubrange = std::any_of(
      hand->subranges().begin(), hand->subranges().end(), [&](auto const& sub) {
        return sub.name() == subrange.name() && sub.color() == subrange.color();
      });
  auto const question =
      QString("Is <b>%1</b> in the <font color=\"%2\"><b>%3</b></font> range?")
...
void Quizer::nextMostPlayedQuiz(HandInfo const* hand, int handIndex) {
  auto const& subrangeInfo = _displayer->currentRange()->subrangeInfo();
  auto subrangeIt =
      std::max_element(hand->subranges().begin(), hand->subranges().end(),
                       [&](auto const& lhs, auto const& rhs) {
                         return lhs.weight() < rhs.weight();
                       });
  // TODO throw?
  if (subrangeIt == hand->subranges().end()) return;
  auto subrangeIndexIt = std::find_if(
      subrangeInfo.begin(), subrangeInfo.end(), [&](auto const& elem) {
        return subrangeIt->name() == elem.name() &&
               subrangeIt->color() == elem.color();
      });
  if (subrangeIndexIt == subrangeInfo.end()) return;
  _correctAnswerButtonIndex =
      std::distance(subrangeInfo.begin(), subrangeIndexIt);
  auto const question =
      QString("What is the most played for <b>%1</b>?").arg(hand->name());
